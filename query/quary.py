...
"""
GET_MEMBERS_ID ="""
    select b.trello_id from boards b
    inner join boards_users bu on b.id = bu.board_id
    inner join lists l on bu.board_id = l.board_id
    inner join cards  c on c.list_id=l.id
    where c.trello_id=%s
"""
# inner join boards_users bu on b.id = bu.board_id
# inner join lists l on l.id=bu.id
# inner join cards c on c.list_id =l.id